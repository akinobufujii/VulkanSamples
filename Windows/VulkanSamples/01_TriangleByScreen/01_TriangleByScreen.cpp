...
static LPCTSTR	CLASS_NAME = TEXT("01_TriangleByScreen");	// ウィンドウネーム
static LPCSTR APPLICATION_NAME = "01_TriangleByScreen";		// アプリケーション名
...
bool createResource()
{
	...
	int vertexBufferSize = vertexBuffer.size() * sizeof(VertexFormat);
	...
	g_vertexBuffer.bindingDescriptions[0].stride = sizeof(VertexFormat);
	g_vertexBuffer.bindingDescriptions[0].inputRate = VK_VERTEX_INPUT_RATE_VERTEX;

	// 頂点属性を設定
	g_vertexBuffer.attributeDescriptions.resize(2);
	...
	g_vertexBuffer.attributeDescriptions[1].format = VK_FORMAT_R32G32B32_SFLOAT;
	g_vertexBuffer.attributeDescriptions[1].offset = sizeof(float) * 3;
	g_vertexBuffer.attributeDescriptions[1].binding = 0;
	...
	shaderStageCreateInfo[0].module = loadShader("triangle.vert.spv", g_VulkanDevice, VK_SHADER_STAGE_VERTEX_BIT);
	...
	shaderStageCreateInfo[1].module = loadShader("triangle.frag.spv", g_VulkanDevice, VK_SHADER_STAGE_FRAGMENT_BIT);
...
void Render()
{
	...
	// 描画コマンド発行
	vkCmdDrawIndexed(command, g_indexBuffer.count, 1, 0, 0, 1);
